...
const FASTING_COLOR = "#F08A24"; // orange, cohérent avec l'écran Jeûne

export default function FastingHoursChart({ data }: Props) {
  const [period, setPeriod] = useState<ChartPeriod>("week");

  const points = useMemo<FastingPoint[]>(() => {
    const sorted: FastingPoint[] = [...data]
      .map((d) => ({
    ...
    return period === "year" ? averagePerMonthFasting(filtered) : filtered;
  }, [data, period]);
  ...
  if (hasData) {
    ...
    const coords = points.map((p, i) => ({ x: toX(i), y: toY(p.hours) }));
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Évolution du jeûne (heures)</Text>
      <PeriodFilterTabs value={period} onChange={setPeriod} />
      {hasData ? chart : <Text style={styles.emptyText}>Aucun jeûne enregistré sur cette période.</Text>}
    </View>
  );
}
